...
/**
 * Represents a storage system for groceries, allowing for adding, removing,
 * displaying, and managing groceries.
 */
        ...
        if (groceries.isEmpty()) {
            ...
            while (amountRemoved != 0) {
                if (groceries.get(Key).getFirst().getAmount() > amountRemoved) {
                    groceries.get(Key).getFirst().setAmount(groceries.get(Key).getFirst().getAmount() - amountRemoved);
                    amountRemoved = 0;
                } else {
    ...
    public List<Grocery> expiredGroceries(LocalDate date) {
        ...
        for (List<Grocery> groceryList : groceries.values()) {
            expiredGroceriesList.addAll(groceryList.stream().filter(g -> g.getExpiryDate().isBefore(date)).toList());
        }
